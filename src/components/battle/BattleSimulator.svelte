<script lang="ts">
	...
	import { calculateTypeWeaknesses, TYPE_EFFECTIVENESS, getTypeColor } from '$lib/utils/pokemon-utils';
	...
	function calculateDamage(attacker: TeamPokemon, defender: TeamPokemon, moveType: string): {
		damage: number;
		effectiveness: number;
		message: string;
	} {
		const attackerAttack = attacker.pokemon.stats.find(s => s.stat.name === 'attack')?.base_stat || 0;
		const defenderDefense = defender.pokemon.stats.find(s => s.stat.name === 'defense')?.base_stat || 0;
	...
	function simulateBattle() {
		...
		battleLog.push(`${pokemon1.nickname || pokemon1.pokemon.name} vs ${pokemon2.nickname || pokemon2.pokemon.name}!`);
		...
		while (hp1 > 0 && hp2 > 0 && turn <= maxTurns) {
			...
			const move1 = firstAttacker.moves[Math.floor(Math.random() * firstAttacker.moves.length)] || 'tackle';
			...
			battleLog.push(`${secondAttacker.nickname || secondAttacker.pokemon.name} took ${result1.damage} damage!`);
			...
			const move2 = secondAttacker.moves[Math.floor(Math.random() * secondAttacker.moves.length)] || 'tackle';
			...
			battleLog.push(`${firstAttacker.nickname || firstAttacker.pokemon.name} took ${result2.damage} damage!`);
			...
			battleLog.push(`${pokemon1.nickname || pokemon1.pokemon.name}: ${Math.max(0, hp1)}/${maxHP1} HP`);
			battleLog.push(`${pokemon2.nickname || pokemon2.pokemon.name}: ${Math.max(0, hp2)}/${maxHP2} HP`);
			battleLog.push('---');
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
		<!-- Team 1 -->
		<div>
			<h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{team1.name}</h3>
			<div class="space-y-3">
				{#each team1.pokemons as teamPokemon}
					<button
						on:click={() => selectPokemon(teamPokemon, 1)}
						class="w-full text-left p-3 rounded-lg border-2 transition-all {selectedPokemon1 === teamPokemon 
		...
		<!-- Team 2 -->
		<div>
			<h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{team2.name}</h3>
			<div class="space-y-3">
				{#each team2.pokemons as teamPokemon}
					<button
						on:click={() => selectPokemon(teamPokemon, 2)}
						class="w-full text-left p-3 rounded-lg border-2 transition-all {selectedPokemon2 === teamPokemon 
	...
	<div class="text-center mb-6">
		<button
			...
			class="px-8 py-3 bg-green-500 hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg font-semibold text-lg transition-colors"
		>
			{#if battleInProgress}
				Battling...
	...
	{#if battleLog.length > 0}
		<div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 max-h-96 overflow-y-auto">
			<h4 class="font-semibold text-gray-900 dark:text-white mb-3">Battle Log</h4>
			<div class="space-y-1 font-mono text-sm">
				{#each battleLog as logEntry}
					<div class="text-gray-700 dark:text-gray-300 {logEntry.includes('wins!') || logEntry.includes('fainted!') ? 'font-bold text-red-600 dark:text-red-400' : ''} {logEntry.includes('super effective') ? 'text-green-600 dark:text-green-400' : ''} {logEntry.includes('not very effective') ? 'text-yellow-600 dark:text-yellow-400' : ''} {logEntry.includes('no effect') ? 'text-gray-500 dark:text-gray-400' : ''}">
						{logEntry}
					</div>
				{/each}
			</div>
